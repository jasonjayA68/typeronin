 ...
 * one ends up not keyboard-reachable. The theatre is layered on top: a sakura
 * glow, a single katana slash sweeping across once, the kanji rising into place,
 * and a gust of petals summoned through the field that is already on the page.
 *
 ...
 *
 * On the "Honor multiplier" the brief asked for: ranks in this game do not carry
 * one — a multiplier is a property of a game mode and the daily rules, not a
 * tier. So the badge shows the tier reached instead, which is the true thing a
 * rank confers.
 */
  ...
  return (
    ...
    >
      <DialogContent className="overflow-hidden border-sakura/30 text-center sm:max-w-md">
        ...
        <DialogTitle className="sr-only">Rank up — you are now {rankUp.name}</DialogTitle>
        ...
        <div className="rank-rise flex flex-col items-center gap-1 py-3">
          ...
          <div className="relative my-3 grid place-items-center">
            ...
            />
            <span className="text-gradient-gold relative font-heading text-6xl leading-none">
              {rankUp.kanji}
            </span>
          </div>

          <p className="font-heading text-2xl font-semibold tracking-wide">{rankUp.name}</p>
          <p className="tabular text-[0.7rem] tracking-[0.18em] text-muted-foreground uppercase">
            Rank {rankUp.tier} of {rankUp.total}
          </p>
